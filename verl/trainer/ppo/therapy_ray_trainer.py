    ...
    if adv_estimator == AdvantageEstimator.GAE:
        ...
        advantages, returns = core_gigpo.compute_gigpo_outcome_advantage(
            token_level_rewards=data.batch[
                "token_level_rewards"
            ],  # for episode group reward computing
...
class TherapyRayPPOTrainer:
    ...
    def build_init_prompt(self,tokenizer, batch_size):
        
        
        
        raw_prompts = ["这只是一个占位的空模版"] * batch_size
        tokenized = tokenizer(raw_prompts, padding=True, truncation=True, return_tensors="pt")
        batch_dict = {
            "input_ids": tokenized["input_ids"],
            "attention_mask": tokenized["attention_mask"],
            "position_ids": tokenized["attention_mask"].clone(),
            "raw_prompt": np.array(raw_prompts, dtype=object),
            "raw_prompt_ids": np.array([tokenizer.encode(p, add_special_tokens=False) for p in raw_prompts], dtype=object),
            "data_source": np.array(["train"] * batch_size, dtype=object)
        }
        
        # print("batch_size",batch_size)
        
        # batch_dict = rl_collate_fn([batch_dict])
        
        # print("batch_dict",batch_dict)
        
        return DataProto.from_single_dict(batch_dict)
    
    def build_dummy_gen_batch(self, tokenizer, batch_size: int) -> DataProto:
        
        bos_id = getattr(tokenizer, "bos_token_id", None)
        if bos_id is None:
            bos_id = getattr(tokenizer, "eos_token_id", 0)
        input_ids = torch.full((batch_size, 1), bos_id, dtype=torch.long)
        attention_mask = torch.ones_like(input_ids)
        # position_ids：很多模型会内部生成；给 0 也可
        ...
        data = {
            ...
            # 这些非张量字段保持形状一致，便于日志或下游用到时不报错
            "raw_prompt": [""] * batch_size,
            "raw_prompt_ids": [[] for _ in range(batch_size)],
            "data_source": ["train"] * batch_size,
        }
        # 如果你的 rl_collate_fn 期望“样本列表”，可按需走一次；否则直接封装即可
        # data = rl_collate_fn([data])
    ...
    def fit(self):
        ...
        # total_training_steps = len(self.train_profiles) * self.config.trainer.total_epochs
            ...
            for batch_dict in self.train_dataloader:
                ...
                batch_profiles = [
                    ClientProfile(
                        ...
                        age=batch_dict["age"][i],
                        gender=batch_dict["gender"][i],
                        job=batch_dict["job"][i],
                        problem=batch_dict["problem"][i],
                        personality=batch_dict["personality"][i],
                        goals=batch_dict["goals"][i]
                    )
                    for i in range(len(batch_dict["name"]))
                ]
                
                # batch_profiles = self.train_profiles[start_idx:start_idx+self.config.data.train_batch_size]
            
                init_prompt_batch = self.build_init_prompt(self.tokenizer, len(batch_profiles) * self.config.env.rollout.n)

                metrics = {}
                timing_raw = {}
                batch: DataProto = init_prompt_batch
