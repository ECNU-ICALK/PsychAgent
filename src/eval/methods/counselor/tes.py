...
TESItemName = Literal[
    ...
    "concern",
    "resonate or capture client feelings",
    "understanding cognitive framework",
    "warmth",
    "inner experience",
    "responsiveness",
    "expressiveness",
    "attuned to client‘s inner world",
]
...
class TES(EvaluationMethod):
    ...
    async def evaluate(self, gpt_api, dialogue: Any, profile: dict = None) -> dict[str, float]:
        ...
        async def run_one(criteria: str) -> list[Item]:
            ...
            for attempt in range(3):
                try:
                    ...
                    expected_item = "inner experience" if criteria == "understanding feelings" else criteria
                    if validated.items[0].item != expected_item:
                        raise ValueError("Mismatched item key for TES criterion")
        ...
        if scores :
            for item in scores:
                mean_score += (float(item.score) - 1.0) / 6.0 * 10.0  # 1-7 -> 0-10
